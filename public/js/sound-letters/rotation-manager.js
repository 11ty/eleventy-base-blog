...
export class RotationManager {
	...
	applyRotation(element, normalizedFreq) {
		...
		const existingRotation =
			parseFloat(element.style.getPropertyValue("--rotation")) || 0;
		...
		if (!this.animationFrame) {
			this.animationFrame = requestAnimationFrame(
				this.updateRotations.bind(this),
			);
		}
	}
	...
	updateRotations(timestamp) {
		this.rotatingElements.forEach((data, element) => {
			if (
				!element.isPressed &&
				Math.abs(data.angularVelocity) < this.rotationSettings.minSpeed
			) {
			...
			data.angularVelocity *= element.isPressed
				? 1
				: Math.pow(this.rotationSettings.friction, deltaTime * 60);

			const normalizedSpeed =
				Math.abs(data.angularVelocity) / this.rotationSettings.maxSpeed;
			const fontWeight = Math.max(
				400,
				Math.min(700, 400 + normalizedSpeed * 300),
			);

			data.rotation += data.angularVelocity * deltaTime;
			element.style.setProperty("--font-weight", fontWeight);
			element.style.setProperty("--rotation", `${data.rotation}deg`); // Add 'deg' unit here
		});
